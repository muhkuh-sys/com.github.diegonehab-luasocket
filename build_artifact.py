...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-luasocket-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-luasocket-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-luasocket-*.xml".')
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-luasocket-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-luasocket-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-luasocket-*.xml".')
